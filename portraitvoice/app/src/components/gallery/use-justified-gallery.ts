...
export function useJustifiedGallery(
  ...
  // Scroll handling is rAF-throttled, while media remains mounted so a fast
  // fling cannot replace already-loaded images with colorful placeholders.
  useEffect(() => {
    const el = viewportRef.current;
    if (el == null) return;
